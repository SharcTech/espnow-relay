...
class ESPythoNow:
    ...
    def __init__(self, interface, mac="", callback=None, accept_broadcast=True, accept_all=False, accept_ack=False,
                 block_on_send=False, pmk="", lmk=""):
        ...
        self.recent_rand_values = collections.deque(
            maxlen=10)  # Ring buffer of recent packet randvalues used to filter packets
    ...
    def prepare(self):
        ...
        self.esp_now_send_packet = scapy.RadioTap() / scapy.Dot11FCS(type=0, subtype=13, addr1=self.local_mac,
                                                                     addr2=self.local_mac,
                                                                     addr3="FF:FF:FF:FF:FF:FF") / scapy.Raw(load=None)
        ...
        if self.encrypted:
            ...
            self.filter = "((type 0 subtype 0xd0%s) or (type 4 subtype 0xd0 and wlan addr1 %s)) and wlan src ! %s" % (
            self_mac_filter, self.local_mac, self.local_mac)
        else:
            # Filter for all unencrypted ESP-NOW messages and ESP-NOW ACK
            self.filter = "((type 0 subtype 0xd0 and wlan[24:4]=0x7f18fe34%s) or (type 4 subtype 0xd0 and wlan addr1 %s)) and wlan src ! %s" % (
            self_mac_filter, self.local_mac, self.local_mac)
    ...
    def start(self):
        ...
        self.listener = scapy.AsyncSniffer(
    ...
    def parse_rx_packet(self, packet):
        ...
        if (not self.accept_all and to_mac != self.local_mac) and (
                not self.accept_broadcast and self.is_broadcast(to_mac)):
            allow = False
        ...
        # Packet is ESP-NOW message
            ...
            if scapy.Dot11CCMP in packet:
                ...
                if self.encrypted:
                    nonce = b'\x00' + bytes.fromhex(from_mac.replace(':', '')) + struct.pack("BBBBBB", packet.PN5,
                                                                                             packet.PN4, packet.PN3,
                                                                                             packet.PN2, packet.PN1,
                                                                                             packet.PN0)
